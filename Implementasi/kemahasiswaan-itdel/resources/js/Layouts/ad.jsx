...
export default function AdminLayout({
    ...
    console.log('Raw Navigation Prop:', navigation);
    ...
    return (
        <div className={`flex min-h-screen transition-colors duration-300 ${theme}`}>
            <Toaster position="top-right" />
            <div className={`
                fixed inset-y-0 left-0 z-50 w-72 overflow-y-auto shadow-lg transform transition-transform duration-300 ease-in-out
                ...
                <div className="flex h-full flex-col">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                <h2 className={`text-xl font-bold bg-clip-text text-transparent
                                    ${theme === 'light' ? 'bg-gradient-to-r from-blue-500 to-indigo-600' : 'bg-gradient-to-r from-blue-400 to-indigo-500'}
                    ...
                    <nav className="flex-1 px-4 py-6 space-y-1">
                        {menuItems.map((item, index) => {
                            ...
                            if (hasSubmenu) {
                                const isSubmenuActive = item.submenu.some(sub => sub.route && isActive(sub.route));
                                return (
                                    <div key={index}>
                                        <button
                                            ...
                                            className={`
                                                ...
                                                ${isSubmenuActive
                                                    ...
                                                    : `${theme === 'light' ? 'text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900' : 'text-zinc-300 hover:bg-zinc-700/50 hover:text-white'}
                                                    ...
                                                    <span className="mr-3">
                                                        <Icon className={`h-5 w-5 transition-all duration-200
                                                            ${isSubmenuActive ? 'text-white' : 
                                                              `${theme === 'light' ? 'text-zinc-500 group-hover:text-blue-500' : 'text-zinc-400 group-hover:text-blue-400'}
                                        ...
                                        {isOpen && (
                                            <div className="pl-8 space-y-1 mt-1">
                                                {item.submenu.map((subItem, subIndex) => {
                                                    if (!subItem.route || !isRouteValid(subItem.route)) return null;
                                                        ...
                                                        <Link
                                                            ...
                                                            className={`
                                                                ...
                                                                ${isSubItemActive
                                                                    ...
                                                                    : `${theme === 'light' ? 'text-zinc-600 hover:bg-zinc-100 hover:text-blue-600' : 'text-zinc-300 hover:bg-zinc-700/50 hover:text-blue-400'}
                            ...
                            if (!item.route || !isRouteValid(item.route)) return null;
                    ...
                    <div className="p-4 mt-auto">
                        <div className={`rounded-xl p-4
                            ${theme === 'light' ? 'bg-zinc-100' : 'bg-zinc-700/30'}
                            ${theme === 'light-blue' ? 'bg-blue-100' : ''}
                            ${theme === 'dark-blue' ? 'bg-blue-800' : ''}`}>
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center w-full px-4 py-2 text-sm transition-all duration-200
                                                ${theme === t.name ? 'bg-blue-500 text-white' : 
                                                  `${theme === 'light' ? 'text-zinc-700 hover:bg-zinc-100' : 'text-zinc-300 hover:bg-zinc-700'}
            ...
            <div className="flex-1 lg:pl-72">
                <header className={`sticky top-0 z-40 backdrop-blur-md shadow-sm
                    ${theme === 'light' ? 'bg-white/90' : 'bg-zinc-800/90'}
                        ...
                        <div className="flex items-center space-x-4">
                            <button
                                className={`lg:hidden p-2 rounded-lg focus:outline-none focus:ring-2
                                    ${theme === 'light' ? 'hover:bg-zinc-100 focus:ring-blue-500' : 'hover:bg-zinc-700 focus:ring-blue-400'}
                            ...
                            <h1 className={`text-lg font-semibold hidden lg:block
                                ...
                                {(() => {
                                    ...
                                    console.log('Processed Menu Items:', menuItems);
                                    const activeMainItem = menuItems.find(item => item.route && isActive(item.route));
                                    if (activeMainItem) {
                                        console.log('Active Main Item:', activeMainItem.name);
                                        return activeMainItem.name;
                                    }

                                    const activeSubItem = menuItems.find(item => 
                                        item.submenu?.some(sub => sub.route && isActive(sub.route))
                                    );
                                    if (activeSubItem) {
                                        const activeSubName = activeSubItem.submenu.find(sub => isActive(sub.route))?.name || activeSubItem.name;
                                        console.log('Active Sub Item:', activeSubName);
                                        return activeSubName;
                                    }
                        ...
                        <div className="flex items-center space-x-2">
                            <button
                                ...
                                className={`p-2 rounded-lg transition-colors hidden lg:block
                                    ${theme === 'light' ? 'hover:bg-zinc-100' : 'hover:bg-zinc-700'}
