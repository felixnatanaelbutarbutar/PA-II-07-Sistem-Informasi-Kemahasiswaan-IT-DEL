...
export default function Edit({ auth, userRole, permissions, menu, category, flash }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post(route('admin.aspiration-category.update', category.id), formData, {
                headers: {
            ...
            setTimeout(() => {
                router.visit(route('admin.aspiration-category.index'));
            }, 1500);
        } catch (error) {
    ...
    return (
        <AdminLayout user={auth.user} userRole={userRole} permissions={permissions} navigation={menu}>
            <Head title="Edit Kategori Aspirasi" />
                ...
                <div className="backdrop-blur-sm bg-white/80 rounded-2xl shadow-lg p-6 mb-8 border border-gray-200/50 flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
                            Edit Kategori Aspirasi
                        </h1>
                        <p className="text-gray-500 mt-1">Perbarui kategori aspirasi</p>
                    </div>
                    <Link
                        href={route('admin.aspiration-category.index')}
