...
export default function Index({ auth, permissions, userRole, menu }) {
    const { achievements = [], flash } = usePage().props ?? {};
            ...
            <div className="py-10 max-w-7xl mx-auto px-4 sm:px-6">
                <div className="backdrop-blur-sm bg-white/80 rounded-2xl shadow-lg p-6 mb-8 border border-gray-200/50">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                        <div>
                            <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Manajemen Prestasi</h1>
                ...
                {isGridView ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {filteredAchievements.map((achievement) => (
                            <div key={achievement.achievement_id} className="group bg-white rounded-xl shadow-lg overflow-hidden flex flex-col h-full transition-all duration-300 hover:shadow-2xl border border-gray-100 hover:border-blue-200 hover:translate-y-[-4px]">
                                <div className="p-5 flex flex-col flex-grow">
                                    <h2 className="text-lg font-bold text-gray-800 mb-2 line-clamp-2 group-hover:text-blue-600 transition">{achievement.title}</h2>
                                    <div className="flex items-center text-sm text-gray-500 mb-3">
                                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 mr-2">
                                            {achievement.category}
                                        </span>
                                        <span>{achievement.achievementType?.type_name || "N/A"}</span>
                                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                        {filteredAchievements.map((achievement, index) => (
                            <div key={achievement.achievement_id} className={`flex flex-col sm:flex-row items-start p-4 hover:bg-gray-50 transition-colors ${index !== filteredAchievements.length - 1 ? 'border-b border-gray-100' : ''}`}>
                                <div className="flex-grow">
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start">
                                        <div>
                                            <h2 className="text-lg font-bold text-gray-800 hover:text-blue-600 transition mb-1">{achievement.title}</h2>
                ...
                {filteredAchievements.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-16 bg-white rounded-xl shadow-lg border border-gray-100">
                        ...
                        <h3 className="text-xl font-medium text-gray-700 mb-2">Tidak ada prestasi yang tersedia</h3>
                        <p className="text-gray-500 text-center max-w-md mb-6">
                            {searchTerm ? "Tidak ada hasil yang cocok dengan pencarian Anda" : "Silahkan tambahkan prestasi baru untuk mulai mengisi konten website Anda"}
                        </p>
                        {searchTerm ? (
                            <button 
