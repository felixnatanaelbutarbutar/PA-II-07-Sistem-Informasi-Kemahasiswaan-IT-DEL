...
export default function Edit({ auth, permissions, userRole, menu, scholarships, form }) {
    const [data, setData] = useState({
        ...
        sections: form.sections.map((section) => ({
            ...
            fields: section.fields.map((field) => ({
                ...
                order: parseInt(field.order) || 1,
                file: null,
                file_path: field.file_path || null,
            })),
        })) || [{ title: '', fields: [{ field_id: null, field_name: '', field_type: 'text', is_required: false, is_active: true, options: '', order: 1, file: null }] }],
    ...
    const addSection = () => {
        setData((prev) => ({
            ...
            sections: [
                ...
                {
                    title: '',
                    fields: [{ field_id: null, field_name: '', field_type: 'text', is_required: false, is_active: true, options: '', order: 1, file: null }],
                },
            ],
        }));
    };
    ...
    const validateField = (sectionIndex, fieldIndex, field) => {
        ...
        if (field.order < 1) {
            ...
            delete newErrors[`sections.${sectionIndex}.fields.${fieldIndex}.order`];
        }
        setErrors(newErrors);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('scholarship_id', data.scholarship_id);
        formData.append('form_name', data.form_name);
        formData.append('description', data.description || '');
        formData.append('is_active', '1'); // Always true
        formData.append('updated_by', data.updated_by);

        data.sections.forEach((section, sectionIndex) => {
            formData.append(`sections[${sectionIndex}][title]`, section.title || '');
            section.fields.forEach((field, fieldIndex) => {
                formData.append(`sections[${sectionIndex}][fields][${fieldIndex}][field_name]`, field.field_name);
                formData.append(`sections[${sectionIndex}][fields][${fieldIndex}][field_type]`, field.field_type);
                formData.append(`sections[${sectionIndex}][fields][${fieldIndex}][is_required]`, field.is_required ? '1' : '0');
                formData.append(`sections[${sectionIndex}][fields][${fieldIndex}][is_active]`, '1'); // Always true
                formData.append(`sections[${sectionIndex}][fields][${fieldIndex}][order]`, field.order);
                formData.append(`sections[${sectionIndex}][fields][${fieldIndex}][options]`, field.options || '');
                if (field.field_id) {
                    formData.append(`sections[${sectionIndex}][fields][${fieldIndex}][field_id]`, field.field_id);
                }
                if (field.file) {
                    formData.append(`sections[${sectionIndex}][fields][${fieldIndex}][file]`, field.file);
                }
            });
        });

        // Debug the route URL
        const updateRoute = route('admin.form.update', form.form_id);
        console.log('Submitting to:', updateRoute);
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6" encType="multipart/form-data" noValidate>
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        setData((prev) => ({ ...prev, form_name: e.target.value }));
                                        setErrors((prev) => ({
                                            ...prev,
                            ...
                            <div className="space-y-2">
                                ...
                                {scholarships.length > 0 ? (
                                    <select
                                        ...
                                        onChange={(e) => {
                                            setData((prev) => ({ ...prev, scholarship_id: e.target.value }));
                                            setErrors((prev) => ({
                                                ...prev,
                                        ...
                                        <div key={fieldIndex} className="space-y-2 mb-4 border-b pb-4">
                                            ...
                                            {field.field_type === 'dropdown' && (
                                                <div className="pl-1/3">
                                                    {(field.options ? field.options.split(',') : ['']).map((option, optIndex) => (
                                                        <div key={optIndex} className="flex items-center space-x-2 mb-2">
                                                            ...
                                                            <button
                                                                ...
                                                                className="text-gray-500 hover:text-red-600"
                                                                disabled={(field.options ? field.options.split(',').length : 1) === 1}
                                                            >
                                                                <svg
                                                                    className="w-5 h-5"
                                                                    fill="none"
