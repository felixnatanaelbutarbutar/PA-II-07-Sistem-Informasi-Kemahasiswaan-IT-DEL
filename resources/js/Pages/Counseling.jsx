...
const COUNSELING_CONFIG = {
  API_URL: 'https://kemahasiswaanitdel.site/api/meta/konseling',
  WHATSAPP_NUMBER: '+6285142232595',
  AVAILABLE_TIMES: ['08:00', '09:00', '10:00', '11:00', '13:00', '14:00', '15:00', '16:00'],
  ALLOWED_ROLES: ['mahasiswa', 'adminbem', 'adminmpm'],
};
...
export default function Counseling({ auth, flash: initialFlash, errors: serverErrors, redirectToLogin }) {
  ...
  const { API_URL, WHATSAPP_NUMBER, AVAILABLE_TIMES } = COUNSELING_CONFIG;
  ...
  // Fetch meta data with abort handling
  useEffect(() => {
    const abortController = new AbortController();
    const fetchMetaData = async () => {
      setIsMetaLoading(true);
      try {
        const metaResponse = await fetch(API_URL, { signal: abortController.signal });
        if (!metaResponse.ok) {
  ...
  // Modal focus trapping and Esc key handling
  useEffect(() => {
  ...
  const handleChange = (e, customErrors = {}) => {
  ...
  const handleLogout = () => {
    router.post(route('logout'), {}, {
      onSuccess: () => router.get(route('login')),
    });
  };

  const handleWhatsAppContact = (e) => {
    e.preventDefault();
    const message = isFormSubmitted
      ? `Halo, saya ${auth.user?.name || 'pengguna'} telah mengisi formulir konseling untuk tanggal ${formData.bookingDate} pukul ${formData.bookingTime}. Bisakah Anda membantu saya untuk langkah selanjutnya?`
      : `Halo, saya ingin bertanya tentang layanan konseling di IT Del.`;
  ...
  const events = useMemo(() => {
    ...
    Object.keys(bookingsByDate).forEach((date) => {
      ...
      if (bookedTimes.length >= AVAILABLE_TIMES.length) {
        eventsList.push({
          title: 'Full Booked',
          start: new Date(date),
          end: new Date(date),
  ...
  const handleSelectSlot = ({ start }) => {
    ...
    const available = AVAILABLE_TIMES.filter((slot) => !bookedTimes.includes(slot));
    ...
    <GuestLayout>
      ...
      <style>
        {`
          ...
          .modal-content .available-slot:hover {
            transform: translateY(-2px);
      ...
      <div className="main-container">
        ...
        {activeTab === 'info' && (
          <div id="info-panel" className="content-section info-section" role="tabpanel">
            {metaError ? (
              ...
              <div className="meta-loading" aria-live="polite">
        ...
        {activeTab === 'calendar' && auth.isMahasiswa && (
          <div id="calendar-panel" className="content-section calendar-section" role="tabpanel">
            ...
            <Calendar
              ...
              aria-label="Kalender jadwal konseling"
            />
            {selectedDate && (
              <div className="modal-overlay" onClick={() => setSelectedDate(null)} role="dialog" aria-modal="true">
                <div
                ...
                >
                      ...
                      <ul>
                        {bookedSlots.map((slot, index) => (
                          <li key={index} className="booked-slot">
                            {slot.time}
                            <div className="slot-info">NIM: {slot.nim}</div>
                            <div className="slot-info">Nama: {slot.name}</div>
                          </li>
                        ))}
                      </ul>
                    </>
                      ...
                      <ul>
                        {availableSlots.map((slot, index) => (
                          <li
                            ...
                            role="button"
                            tabIndex="0"
                            onKeyDown={(e) => e.key === 'Enter' && handleSelectAvailableSlot(slot)}
                            aria-label={`Pilih slot ${slot}`}
                          >
                            {slot}
                          </li>
                        ))}
                  ...
                  {availableSlots.length === 0 && bookedSlots.length === 0 && (
                    <p className="text-gray-600">Tidak ada slot tersedia atau dipesan.</p>
                  )}
                  <button onClick={() => setSelectedDate(null)} aria-label="Tutup modal">
                    Tutup
                  </button>
                </div>
              </div>
        ...
        {activeTab === 'form' && (
          <div id="form-panel" className="content-section form-section" role="tabpanel">
            <h2 id="form-title">Formulir Konseling</h2>
            <p className="subheader">
            ...
            <div className="auth-message" role="alert">
              {auth.isMahasiswa ? (
                ...
                <div>
                  <p className="auth-message">
                    Anda harus login untuk mengirimkan permintaan konseling.
                  </p>
                  <div className="button-group">
                    <Link
                      ...
                      aria-label="Login untuk mengakses formulir konseling"
                    >
                      <img
              ...
              <div className="form-grid">
                <div className="form-group">
                  ...
                  {(errors.issue || serverErrors?.issue) && (
                    <span id="issue-error" className="error">{errors.issue || serverErrors.issue}</span>
                  )}
                </div>
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (selectedDate.getDay() === 0) {
                        handleChange({
                          target: { name: 'bookingDate', value: e.target.value },
                          errors: { bookingDate: 'Tidak dapat melakukan booking pada hari Minggu.' },
                        });
                      } else {
                        handleChange(e);
                      }
                  ...
                  {(errors.bookingDate || serverErrors?.bookingDate) && (
                    <span id="bookingDate-error" className="error">{errors.bookingDate || serverErrors.bookingDate}</span>
                  )}
                </div>
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    required
                    aria-describedby="bookingTime-error"
                  >
                  ...
                  {(errors.bookingTime || serverErrors?.bookingTime) && (
                    <span id="bookingTime-error" className="error">{errors.bookingTime || serverErrors.bookingTime}</span>
                  )}
                </div>
                <div className="form-group">
                  ...
                  <div className="input-container">
                    ...
                    <input
                      ...
                      required
                      aria-describedby="noWhatsApp-error"
                    />
                  </div>
                  {(errors.noWhatsApp || serverErrors?.noWhatsApp) && (
                    <span id="noWhatsApp-error" className="error">{errors.noWhatsApp || serverErrors.noWhatsApp}</span>
                  )}
                </div>
                <div className="button-group">
                  <button
                    type="submit"
                    disabled={!auth.isMahasiswa || isSubmitting}
                    className="submit-button"
                    aria-label="Kirim permintaan konseling"
                  >
                    <img
                      ...
                      alt=""
                    />
                    {isSubmitting ? 'Mengirim...' : 'Kirim Permintaan Konseling'}
                  </button>
                  <button
      ...
      {showSuccess && flash?.success && (
        <div className="success-toast" role="alert" aria-live="assertive">
          <img
            src="https://img.icons8.com/ios-filled/50/ffffff/checkmark.png"
            width="20"
            height="20"
