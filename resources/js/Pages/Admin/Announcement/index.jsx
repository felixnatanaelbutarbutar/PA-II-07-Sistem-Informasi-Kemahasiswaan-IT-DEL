...
export default function Index({ auth, permissions, userRole, menu }) {
    ...
    const handleDeleteClick = (announcement_id) => {
        setAnnouncementIdToDelete(announcement_id);
        setShowDeleteModal(true);
    };

    const confirmDelete = () => {
        if (announcementIdToDelete) {
            const deleteRoute = route("admin.announcement.destroy", announcementIdToDelete);
            console.log('Delete Route:', deleteRoute);

            router.post(deleteRoute, {}, {
                onSuccess: () => {
                    console.log('Delete Success');
                    setNotificationMessage('Pengumuman berhasil dihapus!');
                    setNotificationType('success');
                    setShowNotification(true);
                },
                onError: (errors) => {
                    console.log('Delete Error:', errors);
                    setNotificationMessage('Gagal menghapus pengumuman: ' + (errors.error || 'Terjadi kesalahan.'));
                    setNotificationType('error');
                    setShowNotification(true);
                },
                onFinish: () => {
            ...
            {showDeleteModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md transform transition-all duration-300 scale-100">
                        ...
                        <p className="text-gray-600 text-center mb-6">Apakah Anda yakin ingin menghapus pengumuman ini? Tindakan ini tidak dapat dibatalkan.</p>
            ...
            <div className="py-10 max-w-7xl mx-auto px-4 sm:px-6">
                ...
                <div className="backdrop-blur-sm bg-white/80 rounded-2xl shadow-lg p-6 mb-8 border border-gray-200/50">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                        <div>
                            <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Manajemen Pengumuman</h1>
                            <p className="text-gray-500 mt-1">Kelola konten pengumuman untuk website</p>
                        </div>
                ...
                {isGridView ? (
                    ...
                    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                        {filteredAnnouncement.map((item, index) => (
                            <div
                                key={item.announcement_id}
                                className={`flex flex-col sm:flex-row items-start p-4 hover:bg-gray-50 transition-colors ${index !== filteredAnnouncement.length - 1 ? 'border-b border-gray-100' : ''}`}
                            >
                ...
                {filteredAnnouncement.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-16 bg-white rounded-xl shadow-lg border border-gray-100">
                        ...
                        <h3 className="text-xl font-medium text-gray-700 mb-2">Tidak ada pengumuman yang tersedia</h3>
                        <p className="text-gray-500 text-center max-w-md mb-6">
                            {searchTerm ? "Tidak ada hasil yang cocok dengan pencarian Anda" : "Silahkan tambahkan pengumuman baru untuk mulai mengisi konten website Anda"}
                        </p>
                        {searchTerm ? (
                            <button
