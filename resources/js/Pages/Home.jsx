...
export default function Home() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const newsResponse = await fetch('https://kemahasiswaanitdel.site/api/news?per_page=4');
                if (!newsResponse.ok) throw new Error('Gagal mengambil data berita');
                const newsData = await newsResponse.json();
                setNews(newsData.data || []);

                const categoriesResponse = await fetch('https://kemahasiswaanitdel.site/api/news-categories');
                if (!categoriesResponse.ok) throw new Error('Gagal mengambil data kategori');
                const categoriesData = await categoriesResponse.json();
                setCategories(categoriesData);

                const announcementsResponse = await fetch('https://kemahasiswaanitdel.site/api/announcements?per_page=4');
                if (!announcementsResponse.ok) throw new Error('Gagal mengambil data pengumuman');
                const announcementsData = await announcementsResponse.json();
                setAnnouncements(announcementsData.data || []);

                const activitiesResponse = await fetch('https://kemahasiswaanitdel.site/api/activities/nearest');
                if (!activitiesResponse.ok) throw new Error('Gagal mengambil data kegiatan terdekat');
                const activitiesData = await activitiesResponse.json();
                setActivities(activitiesData || []);

                const achievementsResponse = await fetch('https://kemahasiswaanitdel.site/api/achievements-grouped');
        ...
        const fetchCarousels = async () => {
            ...
            try {
                const carouselResponse = await fetch('https://kemahasiswaanitdel.site/api/carousel/guest');
        ...
        const fetchMetaData = async () => {
            try {
                const metaResponse = await fetch('https://kemahasiswaanitdel.site/api/meta/kata-sambutan');
            ...
            <div className="relative -mt-[1px] bg-gradient-to-r from-blue-900 to-blue-700">
                <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                    <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6">
                        ...
                        >
                            <div className="rounded-full bg-gradient-to-br from-pink-500 via-red-500 to-yellow-500 p-4">
                            ...
                            <span className="hidden sm:block text-sm font-medium">@kemahasiswaanitdel</span>
                        </a>
                        <a
                        ...
                        >
                            <div className="rounded-full bg-black p-4">
                                <svg className="h-8 w-8" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V0h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V3.84a6.84 6.84 0 00-1-.05 6.33 6.33 0 000 12.66 6.33 6.33 0 005.02-.93V24a8.16 8.16 0 004.65-1.49V9.18a4.85 4.85 0 013.76-4.49z" />
                                </svg>
                            </div>
                            <span className="hidden sm:block text-sm font-medium">@it.del</span>
                        </a>
                        <a
                        ...
                        >
                            ...
                            <span className="hidden sm:block text-sm font-medium">Institut Teknologi Del</span>
                        </a>
                        <a
                        ...
                        >
                            ...
                            <span className="hidden sm:block text-sm font-medium">kemahasiswaan@del.ac.id</span>
                        </a>
                    </div>
                </div>
            </div>
